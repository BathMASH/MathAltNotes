		  ...
		  \begin{itemize}
				\item Use partial fractions:
				  \begin{align*}
					...
					\end{align*}
				\item If \(\left|x\right| < a\)%%%% Check syntactic interpretation of verticals
, use \(x = a\tanh(u)\).  Then \(a^2 - x^2 = a^2(1 - \tanh^2(u)) = a^2\sech^2(u)\).
				\item If \(\left|x\right| > a\)%%%% Check syntactic interpretation of verticals
, use \(x = a\coth(u)\), so \(a^2 - x^2 = a^2(1 - \coth^2(u)) = -a^2\cosech^2(u)\).
			\end{itemize}
